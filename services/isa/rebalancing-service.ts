...
export type UserHoldingDetails = {
  etfId?: bigint;
  name: string;
  totalCost: number;
  currentValue: number;
...
export class RebalancingService {
  ...
  async getRebalancingRecommendation(
    ...
    const investmentProfile =
      await this.prismaClient.investmentProfile.findUnique({
        where: { userId },
      });
    ...
    if (!isaAccount) {
      throw new ISAAccountNotFoundError();
    }
    ...
    const recommendedPortfolio =
      RECOMMENDED_PORTFOLIOS[investmentProfile.investType as InvestType];
    const score = this.calculateScore(userPortfolio, recommendedPortfolio);
    const rebalancingOpinions = this.generateRebalancingOpinions(
      userPortfolio,
      recommendedPortfolio,
      userHoldings
    );
  ...
  private mapCategory(categoryPath: string): string | null {
  ...
  private calculateScore(
    ...
    const userMap = new Map(
      userPortfolio.map((p) => [p.category, p.percentage])
    );
  ...
  private generateRebalancingOpinions(
    userPortfolio: UserPortfolio[],
    recommendedPortfolio: { category: string; percentage: number }[],
    ...
    for (const recommended of recommendedPortfolio) {
      ...
      if (diff > threshold) {
        ...
        const holdingsInCategory = userHoldings
          .filter(
            (h) => this.mapCategory(h.categoryPath) === recommended.category
          )
